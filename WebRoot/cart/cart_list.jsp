	...
	<head>
		...
		for(var i=1;i<z.length;i++){
   		...
   		$(function(){
			...
			$("input[name='changeqty']").live('blur',function(){
					var re=new RegExp("\\D+");
					var c=$(this).val();
					...
					calcu();
					var productId = $(this).parent().parent().children().eq(0).text().replace(/\s+/g,"");	
					var qty=$(this).val();
					var thisObj=$(this);
					var getJson=function(){
					$.getJSON("/dangdang/cart/modify.action?dt="+new Date().getTime()+"&productId="+productId+"&newQty="+qty, function(json){			
						if(json.modified){				
					...
					calcu();
						...
						$("a[name='modify']").live('click',function(){
			 ...
			 $(function(){
				$("a[name='delete']").live('click',function(){
					...
					$(this).parent().parent().fadeOut("fast");
						$.getJSON("/dangdang/cart/deletejson.action?dt="+new Date().getTime()+"&productId="+productId, function(json){
						...
						//				
						var tr="<tr><td style='display: none'>"+id+"</td><td width='58' class='buy_td_6'>&nbsp;</td><td width='365' class=t2>&nbsp;&nbsp;<a id='deletedName' href='#'>"+name+"</a></td>"+"<td id='deletedFixedprice'width='106' class='buy_td_5'>"+fixedprice+"</td>"+"<td id='deletedDangprice' width='134' class='buy_td_4'>"+fixedprice+"<span></span></td>"+"<td width='56' class=buy_td_1><a  name='undelete' href='javascript:' src='undelete()'>恢复</a></td>"+"<td width='16' class='objhide'>&nbsp;</td></tr>";
						$("#undeleteTbody").append(tr);
						 $("a[name='undelete']").attr("id",productId);
						 });	
						// $("#divCartItemsRemoved").css("visibility","visible");
						$("#divCartItemsRemoved").fadeIn("slow");
				});
			...
			$("a[name='undelete']").live('click',function(){		
					var productId =$(this).parent().parent().children().eq(0).text().replace(/\s+/g,"");	
					$(this).parent().parent().fadeOut("fast");					
						$.getJSON("/dangdang/cart/undeletejson.action?dt="+new Date().getTime()+"&productId="+productId, function(json){
						...
						//
						var tr="<tr class='td_no_bord' ><td style='display: none'>"+id+"<td class='buy_td_6'><span class='objhide'><img /></span></td><td><a href='#'>"+name+"</a></td><td class='buy_td_5'><span class='c_gray'>￥"+fixedprice+".0</span></td><td class='buy_td_4'>&nbsp;&nbsp;<span>￥"+dangprice+".0</span></td><td class='buy_td_1' id='"+id+"'>&nbsp;&nbsp;"+1+"</td><td ><input name ='changeqty'  class='del_num' type='text' size='3' maxlength='4' value='"+1+"'/><a  id='"+json.product.id+"' name='modify' href='javascript:' class='modifyLink'> 变更</a><span class='modifyTip' style='font-size:10px;color:red'></span></td><td><a name='delete' href='javascript:'>删除</a></td></tr>";   
						$("#tbCartItemsNormal").append(tr);
						 
						 });	
						$("#tbCartItemsNormal").fadeIn("slow");
						//	alert($('#undeleteTbody').children().eq(1).html());
			if($("#undeleteTbody").children().eq(0).html()==null){
				  $("#divCartItemsRemoved").hide();
			}
			});
			...
			<div class="choice_bord" >
				<table class="tabl_buy" id="tbCartItemsNormal">
					<tr class="tabl_buy_title" id="title" >
						...
						<td class="buy_td_1" style='display: none'>
					...
					<s:iterator value="buy" var="sc2" status="st">
                      ...
                      <!-- 购物列表开始 -->
						<tr class='td_no_bord' >
							...
							<td class="buy_td_1" id="qty<s:property value="product.id"/>" style='display: none'>
